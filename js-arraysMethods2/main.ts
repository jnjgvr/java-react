...
const ejercicio5 = () => {
    ...
    let combinedTicket1: Array<string> = [];
...
const ejercicio7 = () => {
    const tarea2: string[] = [...new Set(foods.map(food => food.category))];
    console.log(tarea2);

    const tarea3 = (categoria: string) => tarea2.includes(categoria);
    console.log(tarea3("Italian"));
    console.log(tarea3("Healthy"));
    console.log(tarea3("Spanish"));

    const tarea4 = (categoria: string) => foods.filter((food) => (food.category === categoria));
...
const ejercicio10 = () => {
    ...
    const cafeConLeche = (ticket: Array<string>) => ticket.every((item) => item === '☕');
    ...
    const ticketPersona = (ticket: Array<string>, consumption: Array<string>) => {
    ...
    const ticketPersonaSlice = (ticket: Array<string>, consumption: Array<string>) => {
        const individualTicket = [];
        for (const item of consumption) {
            const index = ticket.indexOf(item);
            if (index !== -1) {
...
const ejercicio11 = () => {
    ...
    const primerBebedor = (bebida:string) => {
    ...
    const ultimoBebedor = (bebida:string) => {
